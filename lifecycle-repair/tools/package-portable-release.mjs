...
export const PORTABLE_RELEASE_NAME_PREFIX = "SelahMC-v8.3.10";
...
async function copyAssetTree(assetRoot, clientRoot, assetOverrides = {}) {
  const knownAssets = new Set(PORTABLE_ASSET_PATHS);
  for (const assetPath of Object.keys(assetOverrides)) {
    if (!knownAssets.has(assetPath)) {
      throw new Error(`portable asset override is not in the manifest: ${assetPath}`);
    }
  }
  for (const assetPath of PORTABLE_ASSET_PATHS) {
    const sourcePath = assetOverrides[assetPath]
      ? resolve(assetOverrides[assetPath])
      : childPath(assetRoot, assetPath);
...
export async function buildPortableRelease(options = {}) {
  ...
  await Promise.all([
    ...
    copyFile(
      join(coreRelease.releaseDirectory, "selahmc-client-v8.3.10.js"),
      join(releaseDirectory, "client", "selahmc-client-v8.3.10.js"),
    ),
    copyWindowsText(
