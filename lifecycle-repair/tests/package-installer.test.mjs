...
async function makeFakeClientRoot(parent) {
  ...
  await writeFile(
    join(target, "selahmc-client-v8.3.10.js"),
    "old-client-v8.3.10\n",
    "utf8",
  );
  await writeFile(
...
test("release contains the versioned client, repaired bridge, and hash-pinned index", async () => {
  ...
  try {
    ...
    assert.match(
      index,
      new RegExp(
        `selahmc-client-v8\\.3\\.10\\.js\\?v=${release.bundleSha256.slice(0, 8)}`,
      ),
    );
    ...
    assert.deepEqual(
      ...
      files
        .map((file) => `SelahMC-v8.3.10-Lifecycle-Transaction/${file}`)
        .sort(),
    );
  } finally {
...
test("installer preserves v8.3.3, creates a rollback backup, and installs atomically", async () => {
  ...
  try {
    ...
    assert.equal(
      await readFile(join(backup, "selahmc-client-v8.3.10.js"), "utf8"),
      "old-client-v8.3.10\n",
    );
    ...
    assert.equal(
      (await stat(join(target, "selahmc-client-v8.3.10.js"))).mode & 0o777,
      0o644,
    );
    assert.equal(
    ...
    const clientRollback = result.stdout.indexOf("selahmc-client-v8.3.10.js");
    ...
    assert.equal(
      await readFile(join(target, "selahmc-client-v8.3.10.js"), "utf8"),
      "old-client-v8.3.10\n",
    );
...
test("corrupt package fails before changing the client root", async () => {
  ...
  try {
    ...
    await appendFile(
      join(corruptRelease, "selahmc-client-v8.3.10.js"),
      "\n// corruption\n",
      "utf8",
    );
    const target = await makeFakeClientRoot(directory);
